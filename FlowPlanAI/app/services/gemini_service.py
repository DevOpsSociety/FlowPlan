...
**[WBS 구조 가이드라인]**
다음 7단계의 표준 프로세스 흐름을 따르되, **상위 작업(Level 0)의 이름과 하위 작업(Level 1)의 내용은 프로젝트의 특성({data.get('project_type')})을 반영하여 구체적으로 작성**하세요.
템플릿의 이름을 기계적으로 쓰지 말고, 프로젝트 문맥에 맞게 자연스럽게 수정하세요.

1. **기획 및 준비 단계**: (예: '{data.get('project_name')} 기획', '요구사항 정의')
2. **분석 및 설계 단계**: (예: '시스템 아키텍처 설계', 'UI/UX 디자인')
3. **환경 구축 단계**: (예: '개발 서버 세팅', '클라우드 인프라 구축')
4. **구현 단계**: (예: '핵심 기능 개발', '프론트엔드/백엔드 구현')
5. **테스트 단계**: (예: 'QA 및 버그 수정', '통합 테스트')
6. **배포 단계**: (예: '서비스 런칭', '스토어 등록')
7. **유지보수 단계**: (예: '서비스 안정화', '모니터링')

필수 규칙:
1. 모든 필드는 영문 키 이름 사용 (project_name, total_tasks, wbs_structure 등)
2. task_id는 "1.0", "1.1" 형식 사용.
   - 최상위 작업(Level 0)은 위 7단계 흐름을 유지하되, 프로젝트에 맞는 구체적인 작업명을 사용하세요.
   - 하위 작업(Level 1)은 각 단계에 필요한 실질적인 세부 작업입니다.
